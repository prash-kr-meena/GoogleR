...
def helper(array_sum, n, x, arr, index, subset_sum) -> bool:
    ...
    # check if already found out earlier
    if memo.get((subset_sum, index)) is not None:
        return memo.get((subset_sum, index))
    ...
    final_answer = answer_when_choosing or answer_when_not_choosing
    memo[(subset_sum, index)] = final_answer
    return final_answer


def water_cooler(n, x, arr):
    array_sum = sum(arr)
    print(helper(array_sum, n, x, arr, 0, 0))
